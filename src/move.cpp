...
ik inverse_kin(double px, double py, double pz, double r11, double r12, double r13, double r21, double r22, double r23, double r31, double r32, double r33)
{
  ...
  //  printf("%lf %lf %lf %lf\n", L, s3, c3, c1) ;
    ...
    if(abs(P.px-px)<1.0e-10 && abs(P.py-py)<1.0e-10 && abs(P.pz-pz)<1.0e-10){
      T.t[0] = t1;
      T.t[1] = t2;
      T.t[2] = t3;
      T.t[3] = t4;
...
//csv2vectorの実行
    ...
    for(int i = 0; i < SIZE ; i+=N){
      ...
      right[i] = inverse_kin(stod(data[i+S][8]) - stod(data[i+S][2]) +80, -stod(data[i+S][9]) + stod(data[0][3]) - 350/*+ 245*/, -stod(data[i+S][10]) + 1400 + 700,0,0,1,0,1,0,-1,0,0);
       //std::vector<std::vector<double>> right
      left[i] = inverse_kin(stod(data[i+S][17]) - stod(data[i+S][11]) +80, -stod(data[i+S][18]) + stod(data[0][12]) +350, -stod(data[i+S][19]) + 1400 + 700,0,0,1,0,1,0,-1,0,0);
...
std::cout << SIZE << std::endl;
   ...
   arm.startJointTrajectory(arm.PositionTrajectory(right,left,0.01));
    ...
    time_out = arm.joint_client_->waitForResult(ros::Duration(100.0));
