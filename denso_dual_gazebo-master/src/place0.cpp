...
//マニピュレータのz方向の位置を決定     これだと先端が範囲最大になった時カバーできてなくない？
...
while (Z <= max_z) {
          ...
          if ( abs( stod(data[i][10]) - stod(data[j][10]) ) < a) {   //範囲内に２点ともある場合

            if(range_xy < sqrt(pow(stod(data[i][8]) - stod(data[j][8]),2) + pow(stod(data[i][9]) - stod(data[j][9]),2))){
              range_xy = sqrt(pow(stod(data[i][8]) - stod(data[j][8]),2) + pow(stod(data[i][9]) - stod(data[j][9]),2));     //範囲の最大値を更新
              z_2 = stod(data[i][10]);
            }
/*          ???
            if (range_xy = max_xy - min_xy) {
              printf("need more manipulator[xy]\n" );
            }
*/
          }
        }
  ...
  while (Y <= min_y) {
    ...
    for (int i = 0; i < SIZE; i++) {
      for (int j = 0; j < SIZE; j++) {
          ...
          if ( X <= stod(data[i][8]) < X+a && X <= stod(data[j][8]) < X+a && Y <= stod(data[i][9]) < Y+a && Y < stod(data[j][9]) < Y+a) {

            if (range_zz > abs(stod(data[i][10]) - stod(data[j][10]))) {
              range_zz = abs(stod(data[i][10]) - stod(data[j][10]));
              x_2 = X + a/2;
              y_2 = Y + a/2;
            }
...
/*  for (int i=0 < SIZE; i++) {
    for (int j = 0; j < SIZE; j++) {
        ...
        if ( sqrt(pow(stod(data[i][8]) - stod(data[j][8]),2) + pow(stod(data[i][9]) - stod(data[j][9]),2)) > max_xy) {
          max_xy = sqrt(pow(stod(data[i][8]) - stod(data[j][8]),2) + pow(stod(data[i][9]) - stod(data[j][9]),2))
        }
        else if ( sqrt(pow(stod(data[i][8]) - stod(data[j][8]),2) + pow(stod(data[i][9]) - stod(data[j][9]),2)) < min_xy) {
          min_xy = sqrt(pow(stod(data[i][8]) - stod(data[j][8]),2) + pow(stod(data[i][9]) - stod(data[j][9]),2))
        }
...
/////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for(int i = 0; i < SIZE; i+=N){
      ...
      right[i] = inverse_kin(stod(data[i][8]), stod(data[i][9]) - stod(data[0][3]), +stod(data[i][10]),0,0,1,0,1,0,-1,0,0);
      ...
      left[i] = inverse_kin(stod(data[i][17]), stod(data[i][18]) - stod(data[0][12]), +stod(data[i][19]),0,0,1,0,1,0,-1,0,0);

       std::cout << stod(data[i][8])<< ","<<  stod(data[i][9])<< ","<< stod(data[i][10])<< std::endl;
    }
    //二次元配列dataの標準出力への出力
