...
//マニピュレータのz方向の位置を決定     これだと先端が範囲最大になった時カバーできてなくない？
        while (Z <= max_z) {
          ...
          if (Z <= stod(data[i][10]) < Z+a && Z <= stod(data[j][10]) < Z+a) {   //範囲内に２点ともある場合
            ...
            if(range_xy < max_xy - min_xy){
              range_xy = max_xy - min_xy;     //範囲の最大値を更新
              z_2 = Z;
            }
/*          ???
            if (range_xy = max_xy - min_xy) {
      ...
      while (X <= max_x) {
        ...
        while (Y <= max_y) {
          ...
          if (X <= stod(data[i][8]) < X+a && X <= stod(data[j][8]) < X+a && Y <= stod(data[i][9]) < Y+a && Y <= stod(data[j][9]) < Y+a) {
            ...
            if (stod(data[i][10])>max_zz) {
              max_zz = stod(data[i][10]);
            }
            else if (stod(data[i][10])>min_zz) {
              min_zz = stod(data[i][10]);
            }

            if (range_zz > max_zz - min_zz) {
              range_zz = max_zz - min_zz;
              x_2 = X;
              y_2 = Y;
            }
