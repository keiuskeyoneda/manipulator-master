...
printf("bbb\n");
    ...
    /*  if( stod(data[i][10]) - stod(data[0][4]) + 695 > 1290 || stod(data[i][10]) - stod(data[0][4]) + 695 < -350.1){
        ROS_ERROR("z of right is out of range!!!");
        printf("%d\n", i);
        ros::shutdown();
      }

      if( stod(data[i][19]) - stod(data[0][13]) + 695 > 1290 || stod(data[i][19]) - stod(data[0][13]) + 695 < -350.1){
        ROS_ERROR("z of left is out of range!!!");
        printf("%d\n", i);
        ros::shutdown();
      }

      if( abs(stod(data[i][8]) - stod(data[0][2]) +130) > 875){
        ROS_ERROR("x of right is out of range!!!");
        printf("%d\n", i);
        ros::shutdown();
      }

      if( abs(stod(data[i][17]) - stod(data[0][11]) +130) > 875){
        ROS_ERROR("x of left is out of range!!!");
        printf("%d\n", i);
        ros::shutdown();
      }
      */
  ...
  ROS_INFO("Phase 3 starts.");
   ...
   arm.startJointTrajectory(arm.PositionTrajectory(right,left,0.5));
